...
cfg_if! {
    if #[cfg(feature = "ssr")] {
        use sqlx::{Connection, SqliteConnectionOptions};
        // use http::{header::SET_COOKIE, HeaderMap, HeaderValue, StatusCode};

        pub async fn db() -> Result<SqliteConnection, ServerFnError> {
            SqliteConnectionOptions::from_str("sqlite:Todos.db")?.in_memory(true).create_if_missing(true).connect().await.map_err(|e| ServerFnError::ServerError(e.to_string()))?
